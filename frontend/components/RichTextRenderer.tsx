...
const serialize = (children: any) =>
  children.map((node: Node, i: number) => {
    ...
    switch (node.type) {
      ...
      case 'h2':
        return (
          <h2 className='text-[2.6rem]' key={i}>
            {serialize(node.children)}
          </h2>
        );
      case 'h3':
        return (
          <h3 className='text-[2.3rem]' key={i}>
            {serialize(node.children)}
          </h3>
        );
      case 'h4':
        return (
          <h4 className='text-[2rem]' key={i}>
            {serialize(node.children)}
          </h4>
        );
      case 'h5':
        return (
          <h5 className='text-[1.6rem]' key={i}>
            {serialize(node.children)}
          </h5>
        );
      case 'h6':
        return (
          <h6 className='text-[1.2rem]' key={i}>
            {serialize(node.children)}
          </h6>
        );
      case 'quote':
